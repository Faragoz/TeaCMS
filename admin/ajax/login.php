...
try
{
	...
	if($user->Login()):
		...
		$rank = $user->getData('id', 'rank');
		$user->setData('rank', $rank['rank']);
		
		$fuses = $user->userFuses();
		if(!$fuses['admin.login'])
		{
			exit(json_encode(array('error'=>true, 'message'=>'No tienes permiso para ingresar a la administración')));
		}
		else
		{
